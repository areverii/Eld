...
// 	/// </summary>
// 	public class EidolonTracer : ModProjectile
// 	{

// 		public OldForestGod source;
...
namespace Eld.Projectiles
{
	public class EidolonTracer : ModProjectile
	{
		...
		private const float PiBeamDivisor = MathHelper.Pi / OldForestGodHoldout.NumBeams;
		...
		public override void AI() {
			...
			Projectile host = Main.projectile[(int)HostIndex];
			if (Projectile.type != ModContent.ProjectileType<EidolonTracer>() || !host.active || host.type != ModContent.ProjectileType<OldForestGodHoldout>()) {
				Projectile.Kill();
				return;
			}

			// Grab some variables from the host Prism.
			Vector2 hostPrismDir = Vector2.Normalize(host.velocity);
			float chargeRatio = MathHelper.Clamp(host.ai[0] / OldForestGodHoldout.MaxCharge, 0f, 1f);

			// Update the beam's damage every frame based on charge and the host Prism's damage.
			Projectile.damage = (int)(host.damage * GetDamageMultiplier(chargeRatio));

			// The beam cannot strike enemies until the host Prism is at a certain charge level.
			Projectile.friendly = host.ai[0] > OldForestGodHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - OldForestGodHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (host.ai[0] + beamIdOffset * spinRate) / (spinRate * OldForestGodHoldout.NumBeams) * MathHelper.TwoPi;
			...
			float hostPrismAngle = host.velocity.ToRotation();
			...
			Projectile.position += hostPrismDir * 16f + new Vector2(0f, -host.gfxOffY);
			...
			float hitscanBeamLength = PerformBeamHitscan(host, chargeRatio >= 1f);
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / OldForestGodHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
