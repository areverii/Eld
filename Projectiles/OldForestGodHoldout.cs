...
{
	public class OldForestGodHoldout : ModProjectile
	{
		...
		public const float DamageStart = 10f;
		...
		private const float AimResponsiveness = 1.0f;
		...
		public override void AI() {
			...
			// Check if the player is no longer holding the OldForestGod item
            if (!player.active || player.HeldItem.type != ModContent.ItemType<GodOfTheForest>())
            {
                // Kill the projectile if the item is not held
                Projectile.Kill();
                return;
            }
		...
		private void UpdateAnimation() {
			...
			int framesPerAnimationUpdate = 2;
		...
		private void UpdatePlayerVisuals(Player player, Vector2 playerHandPos) {
			...
			Vector2 directionToMouse = Main.MouseWorld - player.MountedCenter;

			// Determine the direction the player should face based on the mouse position relative to the player.
			player.ChangeDir(Main.MouseWorld.X > player.position.X ? 1 : -1);

			// Correct the rotation based on the direction to the mouse.
			Projectile.rotation = directionToMouse.ToRotation();

			// Adjust the rotation based on player direction.
			// When facing left, mirror the rotation.
			if (player.direction == -1) {
				Projectile.rotation += MathHelper.Pi;
			}

			// Update player's item rotation to mirror the behavior when facing right.
			player.itemRotation = Projectile.rotation;

			// Adjust the projectile's position based on player's hand position.
			Projectile.Center = playerHandPos;

			// Update the sprite direction based on the player's direction.
			Projectile.spriteDirection = player.direction;

			// Update the player's held item visuals.
		...
		private void FireBeams() {
			...
			for (int b = 0; b < NumBeams; ++b) {
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, beamVelocity, ModContent.ProjectileType<EidolonTracer>(), damage, knockback, Projectile.owner, b, uuid);
			}
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			Color drawColor = GodOfTheForest.OverrideColor;
